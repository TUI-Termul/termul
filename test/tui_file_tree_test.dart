...
import 'package:termul/components/tui_file_tree.dart';
...
void main() {
  Future<void> pumpHost(WidgetTester tester, Widget child) async {
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: SizedBox(width: 280, height: 400, child: child),
        ),
      ),
    );
  }
  ...
  test('tuiFileTreeFlatten expands open folders', () {
    final nodes = tuiFileTreeFlatten(
      ...
      childrenOf: (id) => id == '/a'
          ? const [
              TuiFileNode(id: '/a/c.dart', name: 'c.dart', kind: TuiFileKind.file),
            ]
          : const [],
    );
