...
import 'package:termul/components/tui_progress.dart';
...
void main() {
  ...
  testWidgets('determinate bar reports percent semantics', (tester) async {
    await pumpHost(
      tester,
      under: const TuiProgressBar(value: 0.5),
    );

    expect(
      tester.getSemantics(find.byType(TuiProgressBar)),
      matchesSemantics(
        label: 'Progress 50 percent',
        value: '0.50',
      ),
    );
  });
  ...
  testWidgets('glyph spinner cycles frames', (tester) async {
    await pumpHost(
      tester,
      under: const TuiSpinner(label: 'Loading list'),
    );
