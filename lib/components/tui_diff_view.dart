  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final useSplit =
            split ?? constraints.maxWidth >= splitBreakpoint;
      ...
      child: Column(
        ...
        children: [
          _FileHeader(
            file: file,
            onToggleFold:
                onToggleFold == null ? null : () => onToggleFold!(index),
            onCopyPath: onCopyPath == null ? null : () => onCopyPath!(index),
            onMenu: onFileMenu == null
                ? null
                : (at) => onFileMenu!(index, at),
          ),
          if (file.binary)
            ...
            for (final row in file.rows)
              switch (row) {
                TuiDiffHunkRow(:final header, :final onExpandAbove, :final onExpandBelow) =>
                  _HunkHeader(
                    header: header,
                ...
                TuiDiffSplitRow(:final oldLine, :final newLine) => split
...
TuiDiffLine _asUnified(TuiDiffLine? oldLine, TuiDiffLine? newLine) {
  ...
  return oldLine ??
      const TuiDiffLine(kind: TuiDiffLineKind.context, text: '');
}
      ...
      child: Row(
        children: [
          if (onExpandAbove != null)
            _ExpandBtn(label: '▲', tooltip: 'Expand above', onTap: onExpandAbove!),
          Expanded(
            child: Text(
              header,
              style: TextStyle(
          ...
          if (onExpandBelow != null)
            _ExpandBtn(label: '▼', tooltip: 'Expand below', onTap: onExpandBelow!),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return IntrinsicHeight(
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Expanded(child: _Side(line: oldLine, fallbackKind: TuiDiffLineKind.removed)),
          VerticalDivider(width: 1, thickness: 1, color: p.border),
          Expanded(child: _Side(line: newLine, fallbackKind: TuiDiffLineKind.added)),
        ],
      ),
    );
  }
...
Color _tint(TermulPalette p, TuiDiffLineKind kind) => switch (kind) {
      TuiDiffLineKind.added =>
        p.green.withValues(alpha: p.isLight ? 0.12 : 0.2),
      TuiDiffLineKind.removed =>
        p.red.withValues(alpha: p.isLight ? 0.12 : 0.22),
